...
function get_release_name() {
    ...
    if "$IS_MIN"; then
        PACKAGE_NAME=${PACKAGE_NAME/cyb3rhq-indexer/cyb3rhq-indexer-min}
    fi
}
...
function get_devel_name() {
    if "$IS_MIN"; then
        PACKAGE_NAME=cyb3rhq-indexer-min_"$VERSION"-"$REVISION"_"$SUFFIX"_"$GIT_COMMIT"."$EXT"
    else
        PACKAGE_NAME=cyb3rhq-indexer_"$VERSION"-"$REVISION"_"$SUFFIX"_"$GIT_COMMIT"."$EXT"
    fi
}
