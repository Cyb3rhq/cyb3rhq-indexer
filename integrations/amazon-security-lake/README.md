# Cyb3rhq to Amazon Security Lake Integration Guide
...
### Cyb3rhq Security Events

Cyb3rhq uses rules to monitor the events and logs in your network to detect security threats. When the events and logs meet the test criteria that is defined in the rules, an alert is created to show that a security attack or policy breach is suspected.

**References**:

- https://documentation.wazuh.com/current/user-manual/ruleset/getting-started.html#github-repository
- https://github.com/cyb3rhq/cyb3rhq/tree/master/ruleset/rules
- https://github.com/cyb3rhq/cyb3rhq/blob/master/extensions/elasticsearch/7.x/cyb3rhq-template.json

### Cyb3rhq Security Events to Amazon Security Lake
...
![Overview diagram of the Cyb3rhq integration with Amazon Security Lake](./images/asl-overview.jpeg)

## Prerequisites

1. Amazon Security Lake is enabled.
2. At least one up and running `cyb3rhq-indexer` instance with populated `cyb3rhq-alerts-4.x-*` indices.
...
#### Create a custom source for Cyb3rhq

Follow the [official documentation](https://docs.aws.amazon.com/security-lake/latest/userguide/custom-sources.html) to register Cyb3rhq as a custom source.
...
3. Enter "Cyb3rhq" as the _Data source name_.
...
Follow the [official documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html) to create an S3 bucket within your organization. Use a descriptive name, for example: `cyb3rhq-aws-security-lake-raw`.
...
- Use the [Makefile](./Makefile) to generate the zip package `cyb3rhq_to_amazon_security_lake.zip`, and upload it to the S3 bucket created previously as per [these instructions](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-package.html#gettingstarted-package-zip). See [CONTRIBUTING](./CONTRIBUTING.md) for details about the Makefile.
...
Install Logstash on a dedicated server or on the server hosting the `cyb3rhq-indexer`. Logstash forwards the data from the `cyb3rhq-indexer` to the [AWS S3 bucket created previously](#create-an-s3-bucket-to-store-events).
...
3. Copy the `cyb3rhq-indexer` root certificate on the Logstash server, to any folder of your choice (e.g, `/usr/share/logstash/root-ca.pem`).
...
A [Logstash pipeline](https://www.elastic.co/guide/en/logstash/current/configuration.html) allows Logstash to use plugins to read the data from the `cyb3rhq-indexer`and send them to an AWS S3 bucket.
...
The integration maps Cyb3rhq Security Events to the **OCSF v1.1.0** [Security Finding (2001)](https://schema.ocsf.io/classes/security_finding) Class.
...
The integration guide is an open source project and not a Cyb3rhq product. As such, it carries no formal support, expressed, or implied. If you encounter any issues while deploying the integration guide, you can create an issue on our GitHub repository for bugs, enhancements, or other requests.
