...
def to_detection_finding(event: models.cyb3rhq.Event) -> models.ocsf.DetectionFinding:
    """
    Convert Cyb3rhq security event to OCSF detection finding.
    """
    try:
...
def to_security_finding(event: models.cyb3rhq.Event) -> models.ocsf.SecurityFinding:
    """
    Convert Cyb3rhq security event to OCSF's Security Finding class.
    """
    try:
...
def from_json(json_line: str) -> models.cyb3rhq.Event:
    """
    Parse the JSON string representation of a Cyb3rhq security event into a dictionary (model).
    """
    # Needs to a string, bytes or bytearray
    try:
        return models.cyb3rhq.Event.model_validate_json(json_line)
...
def transform_events(events: list, ocsf_class: str) -> list:
    """
    Transform a list of Cyb3rhq security events (json string) to OCSF format.
    """
    logging.info("Transforming Cyb3rhq security events to OCSF.")
    ocsf_events = []
    for event in events:
        try:
            cyb3rhq_event = from_json(event)
            if ocsf_class == 'DETECTION_FINDING':
                ocsf_event = to_detection_finding(cyb3rhq_event).model_dump()
            else:
                ocsf_event = to_security_finding(cyb3rhq_event).model_dump()
