...
# Constants
INDEXER_HOME=/usr/share/cyb3rhq-indexer
...
# is idempotent.
source /usr/share/cyb3rhq-indexer/bin/opensearch-env-from-file
...
if [[ -f bin/opensearch-users ]]; then
  ...
  if [[ -n "$INDEXER_PASSWORD" ]]; then
    [[ -f /usr/share/cyb3rhq-indexer/opensearch.keystore ]] || (run_as_other_user_if_needed opensearch-keystore create)
...
if [[ "$(id -u)" == "0" ]]; then
  ...
  if [[ -n "$TAKE_FILE_OWNERSHIP" ]]; then
    chown -R 1000:0 /usr/share/cyb3rhq-indexer/{data,logs}
  fi
fi
...
#if [[ "$DISCOVERY" == "single-node" ]] && [[ ! -f "/var/lib/cyb3rhq-indexer/.flag" ]]; then
# run securityadmin.sh for single node with CACERT, CERT and KEY parameter
#  nohup /securityadmin.sh &
#  touch "/var/lib/cyb3rhq-indexer/.flag"
#fi

run_as_other_user_if_needed /usr/share/cyb3rhq-indexer/bin/opensearch <<<"$KEYSTORE_PASSWORD"
